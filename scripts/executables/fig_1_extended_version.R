...
cairo_pdf("../../figures/fig1_extended.pdf", width=6.5, height=8.4)
...
for (i in angle) {
  ...
  oneplot.allellipse(sims.dir, G.factor=G.factor, S.factor=S.factor, M.factor=M.factor, 
                     ...
                     yaxt = "n", xaxt = "n", mgp = c(1, 1, 0), asp=1)
...
w_Eva <-round( 1- mean((modulo.all((df.w$ang_M - df.m$ang_S) ,modulo=pi)^2)/((pi^2)/12)),3)

#Print of Xi_alpha values for each model
