...
for (i in sims.dir) {
   templist <- list.files(path=i, pattern='template.*\\.temp', full.names = TRUE)
        for (temp in templist) {
          generate.param.list(template=temp, param.list=list(FITNESS_OPTIMUM=c(-1, -0.5, 0, 0.5, 1),
                                                             FITNESS_OPTIMUM=c(-1, -0.5, 0, 0.5, 1)), vec.indx=c(1,2), simevolv=sim ,reps=13, launchfile="launcher.sh")
        }
}
...
###############################################################################################################################
sims.dirs <- c(
  "../../simul/fig_3/raster_initcorrneg"
)
...
###############################################################################################################################
sims.dirs <- c(
  "../../simul/fig_3/raster_initcorrpos"
)
